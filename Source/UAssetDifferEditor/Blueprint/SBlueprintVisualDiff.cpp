...
#include "UAssetDifferEditor/AssetDifferClipboardData.h"
...
void SBlueprintVisualDiff::ConstructSuper()
{
	...
	PanelOld.RevisionInfo = FRevisionInfo { FString::Printf(TEXT("本地工作区资源 %s"), *OldBlueprint->GetName()) };
	PanelNew.RevisionInfo = FRevisionInfo { FString::Printf(TEXT("对比的资源     %s"), *OldBlueprint->GetName()) };
	...
	TopRevisionInfoWidget =
		...
		[
			...
			[
				...
				TextBlock(FText::FromString(PanelOld.RevisionInfo.Revision))
			]
			+ SSplitter::Slot()
			.Value(.5f)
			[
				// TextBlock(DiffViewUtils::GetPanelLabel(PanelNew.Blueprint, PanelNew.RevisionInfo, FText()))
				TextBlock(FText::FromString(PanelNew.RevisionInfo.Revision))
			]
		];
	...
	const FSlateBrush* UseSelectionFromContentBrowserBrush = FAppStyle::GetBrush( "Docking.Tab", ".ContentAreaBrush" );
					...
					[
						...
						[
							...
							[
								...
								[
									SNew(SImage)
									.Image(FAppStyle::Get().GetBrush("ContentReference.UseSelectionFromContentBrowser"))
								]
								+ SVerticalBox::Slot()
								.HAlign(HAlign_Center)
...
void SBlueprintVisualDiff::AddFunctionGraph(UBlueprint* Blueprint, UEdGraph* Graph)
{
	...
	FAssetDifferClipboardData ClipboardData(Graph);
	...
	// 粘贴函数
	FAssetDifferClipboardData FuncData;
	FStringOutputDevice Errors;
	FString ClipboardText;
	FPlatformApplicationMisc::ClipboardPaste(ClipboardText);
	const TCHAR* Import = ClipboardText.GetCharArray().GetData() + FCString::Strlen(::GRAPH_PREFIX);
	FAssetDifferClipboardData::StaticStruct()->ImportText(Import, &FuncData, nullptr, 0, &Errors, FAssetDifferClipboardData::StaticStruct()->GetName());
...
#endif
	bool Ret = UAssetDifferDelegate::OnBlueprintDiffWidgetClosed.ExecuteIfBound();
	UAssetDifferDelegate::OnBlueprintDiffWidgetClosed.Unbind();
}
